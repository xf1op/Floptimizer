...
### All services here are DEFAULT, if you need to reset them take this list for yourself.
...
| AllJoyn Router Service | Manual (Trigger Start) | Enables AllJoyn devices and apps to communicate with each other regardless of manufacturer or platform. |
...
| Auto Time Zone Updater | Disabled | Automatically sets the time zone of the device based on location. |
...
| BranchCache | Manual | Caches content from file and web servers on a local subnet. |
...
| Connected Devices Platform Service | Automatic (Delayed Start, Trigger Start) | Enables discovery and interaction with connected devices. |
| Connected Devices Platform User Service | Automatic | Supports user-specific device connections. |
| Connected User Experiences and Telemetry | Automatic | Manages diagnostics and feedback. |
| ConsentUX | Manual | Handles consent prompts for apps and Windows features. |
| Contact Data | Manual | Provides contact data services for apps. |
...
| Device Management Wireless Application Protocol (WAP) Push message Routing Service | Manual (Trigger Start) | Routes WAP push messages for mobile devices. |
...
| Diagnostic Policy Service | Automatic | Detects and troubleshoots problems. |
...
| Display Policy Service | Automatic (Delayed Start) | Manages display-related policies. |
| Distributed Link Tracking Client | Automatic | Maintains links to files on NTFS volumes across a network. |
...
| Downloaded Maps Manager | Automatic (Delayed Start) | Manages offline map data. |
...
| Fax | Manual | Provides fax service support. |
| File History Service | Automatic (Delayed Start, Trigger Start) | Manages file backups. |
| Function Discovery Provider Host | Manual | Helps discover network devices. |
| Function Discovery Resource Publication | Manual (Trigger Start) | Publishes device info to the network. |
| GameDVR and Broadcast User Service | Manual | Provides user mode support for Game DVR and broadcasting. |
| GameInput Service | Manual (Trigger Start) | Supports input devices for gaming. |
| Geolocation Service | Manual (Trigger Start) | Provides location data for apps and services. |
...
| IP Helper | Automatic | Provides tunnel connectivity using IPv6 transition technologies. |
...
| Microsoft (R) Diagnostics Hub Standard Collector Service | Manual | Collects diagnostic information for troubleshooting. |
| Microsoft Account Sign-in Assistant | Manual (Trigger Start) | Supports Microsoft account sign-in processes. |
| Microsoft App-V Client | Disabled | Supports virtualized application delivery. |
...
| Microsoft Edge Elevation Service (MicrosoftEdgeElevationService) | Manual | Manages elevation requests for Microsoft Edge. |
| Microsoft Edge Update Service (edgeupdate) | Automatic (Delayed Start, Trigger Start) | Updates Microsoft Edge automatically. |
| Microsoft Edge Update Service (edgeupdatem) | Manual (Trigger Start) | Manages Microsoft Edge updates in the background. |
...
| Net.Tcp Port Sharing Service | Disabled | Shares TCP ports for WCF services. |
| Netlogon | Manual | Supports domain logon and authentication. |
...
| OpenSSH Authentication Agent | Disabled | Provides SSH authentication agent functionality. |
| OpenSSH SSH Server | Manual | Provides SSH server capabilities. |
| Optimize drives | Manual | Manages disk optimization and defragmentation. |
| Parental Controls | Manual | Manages parental control settings. |
...
| Phone Service | Manual (Trigger Start) | Manages telephony services. |
...
| Print Spooler | Automatic | Manages printing and spooling print jobs. |
...
| Problem Reports Control Panel Support | Manual | Supports the problem reporting interface. |
| Program Compatibility Assistant Service | Manual | Assists in application compatibility issues. |
| Quality Windows Audio Video Experience | Manual | Enhances audio and video quality. |
| Radio Management Service | Manual | Manages wireless radio devices. |
...
| Server | Automatic (Trigger Start) | Provides file, print, and named pipe sharing. |
| Shared PC Account Manager | Disabled | Manages accounts for shared PC scenarios. |
| Shell Hardware Detection | Automatic | Detects hardware events like media insertion. |
...
| SysMain | Automatic | Improves system performance over time. |
| System Event Notification Service | Automatic | Notifies system events and logon/logoff. |
...
| TCP/IP NetBIOS Helper | Manual (Trigger Start) | Provides NetBIOS over TCP/IP support. |
| Telephony | Manual | Manages telephony services and devices. |
| Themes | Automatic | Manages themes and visual styles. |
...
| Update Orchestrator Service | Automatic (Delayed Start) | Manages Windows update orchestration. |
| UPnP Device Host | Manual | Hosts UPnP devices on the network. |
| User Data Access | Manual | Manages user data access for apps. |
| User Data Storage | Manual | Stores user data for applications. |
...
| Windows Font Cache Service | Automatic | Caches font data for faster access. |
| Windows Image Acquisition (WIA) | Manual (Trigger Start) | Manages imaging devices like scanners. |
...
| Windows Push Notifications System Service | Automatic | Manages push notifications for apps. |
| Windows Push Notifications User Service | Automatic | Handles user-specific push notifications. |
...
| Windows Search | Automatic (Delayed Start) | Provides content indexing and search. |
...
| Work Folders | Manual | Syncs user files to corporate servers. |
| Workstation | Automatic | Provides network connections and communications. |
